...
public class ProductController {
    ...
    // GET produit par ID avec variantes
    @GetMapping("/{id}")
    ...
    // GET liste de produits avec pagination et filtres
    @GetMapping
    public ResponseEntity<Page<ProductResponse>> getProducts(
        ...
        if(brandId != null && typeId != null && keyword != null && !keyword.isEmpty()) {
            List<ProductResponse> products = productService.searchProductsByBrandTypeAndName(brandId, typeId, keyword);
            productResponsePage = new PageImpl<>(products, pageable, products.size());
        } else if(brandId != null && typeId != null) {
            List<ProductResponse> products = productService.searchProductsByBrandAndType(brandId, typeId);
            productResponsePage = new PageImpl<>(products, pageable, products.size());
        } else if(brandId != null) {
            List<ProductResponse> products = productService.searchProductsByBrand(brandId);
            productResponsePage = new PageImpl<>(products, pageable, products.size());
        } else if(typeId != null) {
            List<ProductResponse> products = productService.searchProductsByType(typeId);
            productResponsePage = new PageImpl<>(products, pageable, products.size());
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductResponse> updateProduct(@PathVariable("id") Integer productId,
